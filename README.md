...
| **Rogue** | Has 20% less maximum health, takes 40% more damage, carries 50% less ammunition, gains extra native critical chance only with Rogue weapons, and uses Shadow Veil for powerful knife/pistol ambushes |
...
- A permanent first-perk class choice: Tank (300 base HP, 50% protection, -50% damage, +100% ammo, and Overdrive), Healer (team-wide 5 HP healing every two seconds, -25% damage, +25% ammo, and a V-key Field Supply toss), Executioner (+30% damage, -25% maximum health, Judgment and Death Sentence), Doom Guy (+10% damage, 10% protection, regeneration, and Blood Punch), Rogue (20% lower maximum health, 40% more incoming damage, 50% lower maximum ammo, and an actively triggered Shadow Veil), or Engineer (a deployable physical-tracer auto-turret). The choice costs one perk point.
- The Rogue Knife and Silenced Pistol receive +5% native critical chance; other weapons do not receive this class bonus. Their Rogue Class Training adds another +2% per rank only while either Rogue weapon is equipped. Rogue critical hits, including forced Ambush criticals, inflict eight seconds of bleeding that repeats the triggering critical hit's total damage over eight ticks, capped at 24% of the target's RPG-scaled maximum health. Ordinary critical Bleeding cannot refresh or stack, and the monster resists it for 12 seconds after it ends. Every Rogue Knife hit is the exception: its Bleed accumulates and refreshes, adding twice the hit's damage normally or four times its damage from a Shadow Veil ambush, with respective 24% and 36% health ceilings. On iconic episode-end bosses, those stackable knife ceilings are reduced to 10% normally and 15% from stealth. The knife now reaches 80 units before training; each training rank adds 20% reach and 25% attack speed. Critical hits still deal normal critical damage during these states. Bleeding appears on the target display and overhead health bar, with red floating damage numbers.
...
- Rogue Shadow Veil makes the Rogue half translucent and untargetable for eighteen seconds. Monsters actively hunting that Rogue are handed an invisible dead decoy, so their native Doom AI concludes that its target died and stops naturally. Monsters fighting another player are unaffected. Attacking, taking damage, expiry, or pressing the ability again ends it. Its ambush attack deals x4 ranged damage or x10 knife damage, raised to x6 and x15 by the Class Ultimate. Fists, knuckles and other melee weapons receive no special Ambush multiplier. Iconic finale bosses use reduced x2 ranged and x5 knife multipliers, raised to x3 and x7 by the Ultimate. Shadow Veil begins ready, but after use it can only be restored by dealing ordinary weapon damage. A full charge requires `400 + 60 × player level` damage, before Class Training bonuses. Ambush and Bleeding damage do not refill the ability, preventing immediate loops, but sustained damage to a boss can earn another Veil during that fight. Waiting provides no charge. Each Class Training rank adds six seconds of duration and 15% more charge from damage.
