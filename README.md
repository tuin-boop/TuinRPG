...
To test Blood Punch instantly, enter `netevent tuin_test_blood_punch`. This testing command selects Doom Guy and fills Blood Punch; hold and release `V` to use it, then repeat the command whenever another full charge is needed. Because v0.6.30 changes Class Ability from a press command to a press/release binding, existing players should bind **Class Ability** to `V` once under **Customize Controls > Tuin RPG** after updating.
