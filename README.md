...
- Doom Guy also receives the class-exclusive **Quad Shotgun** in slot 3, including Complex Doom's original first-person sequence and `QSGP` world pickup sprite. Primary fire spends four shells on a 52-pellet blast; secondary fire spends two shells on a 20-pellet blast. The primary deliberately uses Complex Doom's ordinary double-barrel sound instead of its much louder quad-blast sample. Existing Doom Guy saves receive the weapon automatically, its RPG variants can drop normally once owned, and other classes see a **DOOM GUY CLASS ONLY** warning instead of consuming one.
