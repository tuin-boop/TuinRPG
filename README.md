...
- Rogues have +5% innate critical chance. Their critical hits, including forced Ambush criticals, inflict eight seconds of bleeding for 4% of the target's RPG-scaled maximum health each second; new criticals refresh rather than stack the duration. Bleeding appears on the target display and overhead health bar, with red floating damage numbers. Rogue Class Training adds another +2% critical chance per rank.
- Press the configurable **Rogue Shadow Veil** key (`V` by default) to become half translucent and untargetable for six seconds, forcing monsters to disengage and wander. Attacking, taking damage, expiry, or pressing the ability again ends it. Its ambush attack deals x4 weapon damage or x20 fist/knuckle damage; iconic finale bosses use reduced x2 ranged and x5 melee multipliers. The base cooldown is 15 seconds. Criticals refund one second, kills refund three, and Ambush kills refund five total. Standing still doubles passive cooldown recovery but is not required. Each Class Training rank adds two seconds of duration and removes two seconds from the base cooldown; rank III adds another three-second Ambush-kill refund.
