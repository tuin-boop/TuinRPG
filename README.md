...
| **Executioner** | Deals 30% more damage, has 25% less maximum health, takes 10% more damage, and spends damage-powered Judgment to sentence priority targets |
...
- A permanent first-perk class choice: Tank (300 base HP, 50% protection, -50% damage, +100% ammo, and Overdrive), Healer (team-wide 5 HP healing every two seconds, -25% damage, +25% ammo, and a V-key Field Supply toss), Executioner (+30% damage, -25% maximum health, +10% damage taken, Judgment and Death Sentence), Doom Guy (+10% damage, 10% protection, regeneration, and Blood Punch), or Rogue (20% lower maximum health and an actively triggered Shadow Veil). The choice costs one perk point.
...
- Press the configurable **Class Ability** key (`V` by default) for the Tank's Overdrive, Healer's Field Supply, Executioner's Death Sentence, Doom Guy's Blood Punch, or Rogue's Shadow Veil. Field Supply throws one Health Bonus, Stimpack, or Medikit forward every 20 seconds, with a 2% Soulsphere chance and a visible cooldown timer. Tank Overdrive charges from damage dealt plus four times health actually lost; its threshold is `800 + 50 × player level`. At full charge, `V` grants +150% weapon damage and firing speed for ten seconds with a red glow. It cannot recharge while active and empties when it ends. Normal combined firing-speed bonuses remain capped at +75%, while Overdrive safely raises that combined cap to +200%.
- Executioner weapon damage fills Judgment, requiring `500 + 50 × player level` damage before Class Training bonuses. At full charge, the HUD asks the Executioner to choose a target; aim at a monster and press `V` to place Death Sentence for ten seconds. A custom blood-red skull bobs and pulses above the sentenced monster. The marked target takes 25% more damage, reduced to 10% against Boss-tier monsters. Killing it while marked triggers Final Verdict, which uses the quick grenade's explosion visuals and damages other visible hostiles within 160 units for 18% of the executed monster's scaled maximum health. Final Verdict damage is clamped from 40 to 400. The **No Appeals** Class Ultimate extends the sentence to twelve seconds, raises the bonuses to 35% and 15%, expands Final Verdict to 224 units at 25% health with a 60-600 clamp, and refunds 25% Judgment after the execution.
...
- Later perk points purchase three ranks of Vital Core (+10 HP/rank), Scavenger (+10% ammo/rank), Killer Instinct (+2% critical chance/rank), Iron Skin (-3% incoming damage/rank), Blood Drinker (1% damage leech/rank), and a class-specific specialty. A level-20 Class Ultimate requires Class Training II.
...
To test Death Sentence instantly, enter `netevent tuin_test_death_sentence`. This selects Executioner and fills Judgment; aim at a monster and press `V` to mark it.
