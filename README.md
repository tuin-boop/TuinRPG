...
- Overdrive firing speed uses the engine's universal weapon-state accelerator, so slow launchers and one-tic custom weapon animations receive the boost consistently. Grenade explosions that are completely swallowed by the Cyberdemon or Spider Mastermind's native radius immunity receive one fallback boss-impact hit without duplicating grenades that already dealt damage.
